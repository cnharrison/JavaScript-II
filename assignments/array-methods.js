...
// Problem 1: boss needs a CSV of ids and email addresses of each person on the list to input into intercom to send out a mailer. 
let mailer = [];
runners.forEach(function(element) {
  mailer.push(element.id + "," + element.email);
});
console.log(mailer);


// Problem 2 boss needs list of people who have donated less than $100 so he knows who to bother for a bigger donation next time 
const over100 = runners.filter(function (element) { 
...
// Problem 3 boss needs a CSV of each compmany that participated in the event to created a graphic on the promotional website for the event
let companyArray = []
runners.forEach(function(element) { 
    if (companyArray.indexOf(element.company_name) === -1) {
